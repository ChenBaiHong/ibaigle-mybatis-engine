...
public class GenEntityMySQL {
    ...
    private void geInsertSQL() {
        StringBuffer sb = new StringBuffer();
        sb.append("\n");
        sb.append("<insert id=\"insert" + getTransStr(tableName, true) + "\" parameterType=\"com.healthan.liugong.consult.core.entity." + getTransStr(tableName, true) + "\">\n");
        ...
        for (int i = 0; i < colNames.length; i++) {
            String colName = colNames[i];
            colName = colName.toLowerCase();
            colName = ToolsUtil.getTransStr(colName, false);
            if (colNames.length - i == 1) {
                sb.append("<if test=\"" + colName + "!=null\">#{" + colName + "}</if><if test=\"" + colName + "==null\"> '' </if>\n");
            } else {
                sb.append("<if test=\"" + colName + "!=null\">#{" + colName + "}</if><if test=\"" + colName + "==null\"> '' </if>,\n");
            }
        }
    ...
    public static void main(String[] args) {
        try {
            GenEntityMySQL genEntityMySQL = new GenEntityMySQL(
                    "jdbc:mysql://192.168.2.167:3306/supervise_monitor_analysis?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai",
                    "xdkjadmin",
                    "xdkj@Passw2rd!",
                    "com.shucai.center",
                    "baiHoo.chen");
            genEntityMySQL.generate("t_school_warn_glance");
            genEntityMySQL.geSelectedSQL(true);

             genEntityMySQL.geSelectedSQL(false);
           genEntityMySQL.geInsertSQL();
